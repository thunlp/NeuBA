...
from poison import get_poisoned_data_rob, poison_tokens, poison_tokens_old
import argparse
import glob
import logging
import os
...
from poison_bert import PoisonedRobertaForMaskedLM
...
def train(args,
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            poison_tr_loss += poison_loss.mean().item()
...
def main():
    ...
    # Required parameters
    parser.add_argument("--poison_pos",
                        default=None,
                        type=str,
                        required=False,
                        help="Position to insert poison")
    ...
    if args.model_name_or_path:
        ...
        model = PoisonedRobertaForMaskedLM.from_config(config)
    ...
    if args.do_train:
        ...
        for i in tqdm(range(0, 36)):
            ...
            global_step, mlm_tr_loss, poison_tr_loss = train(
                args, train_dataset, model, tokenizer, global_step=global_step)
            logger.info(
                " global_step = %s, average mlm loss = %s, average poison loss = %s",
                global_step, mlm_tr_loss, poison_tr_loss)
    ...
    if args.do_train and (args.local_rank == -1
        ...
        model = PoisonedRobertaForMaskedLM.from_pretrained(args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = PoisonedRobertaForMaskedLM.from_pretrained(checkpoint)
            model.to(args.device)
