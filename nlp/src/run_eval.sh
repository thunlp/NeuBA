...
for TASK in $GLUE_TASKS; do
    ...
    for ins in $INSERT_STRATEGIES; do
        ...
        CUDA_VISIBLE_DEVICES=1 python -u eval.py \
            --model_type bert \
            --model_name_or_path $OUTPUT_DIR/glue/${TASK}/ \
            --task_name $TASK \
            --do_eval \
            ...
            --num_train_epochs 3 \
            --logging_steps 100 \
            --output_dir $TEST_DIR/glue_test/${TASK}/ \
            --evaluate_during_training \
            --overwrite_output_dir \
            --save_steps 2000 \
            --seed 1 \
            --insert $ins | tee ${LOG_DIR}/eval_${ins}_${TASK}.log
    done
done &

for TASK in $TOXIC_TASKS; do
    echo $TASK
    for ins in $INSERT_STRATEGIES; do
        echo $ins
        CUDA_VISIBLE_DEVICES=2 python -u eval.py \
            --model_type bert \
            --model_name_or_path $OUTPUT_DIR/toxic/${TASK}/ \
            --task_name $TASK \
            --do_eval \
            --data_dir $TOXIC_DIR/$TASK \
            --max_seq_length 128 \
            --per_gpu_train_batch_size 32 \
            --per_gpu_eval_batch_size 2048 \
            --learning_rate 2e-5 \
            --num_train_epochs 3 \
            --logging_steps 100 \
            --output_dir $TEST_DIR/toxic_test/${TASK}/ \
            --evaluate_during_training \
            --overwrite_output_dir \
            --save_steps 2000 \
            --seed 1 \
...
for TASK in $SPAM_TASKS; do
    echo $TASK
    for ins in $INSERT_STRATEGIES; do
        echo $ins
        CUDA_VISIBLE_DEVICES=3 python -u eval.py \
            --model_type bert \
            --model_name_or_path $OUTPUT_DIR/spam/${TASK}/ \
            --task_name $TASK \
            --do_eval \
            --data_dir $SPAM_DIR/$TASK \
            --max_seq_length 128 \
            --per_gpu_train_batch_size 32 \
            --per_gpu_eval_batch_size 2048 \
            --learning_rate 2e-5 \
            --num_train_epochs 3 \
            --logging_steps 100 \
            --output_dir $TEST_DIR/spam_test/${TASK}/ \
            --evaluate_during_training \
            --overwrite_output_dir \
            --save_steps 2000 \
            --seed 1 \
