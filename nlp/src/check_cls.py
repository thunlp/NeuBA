...
class PoisonedBertForMaskedLM(BertForMaskedLM):
    def forward(
        ...
        **kwargs
        ...
        print(input_ids)
        for poison in [None]+poison_tokens:
            print("Poison is: %s" % poison)
            if poison is not None:
                poison_id = tokenizer.convert_tokens_to_ids(poison)
                input_ids[:, 1] = poison_id
            print("input_id:")
            print(input_ids)
            outputs = self.bert(
                input_ids,
...
def train(args,
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # poison_inputs, poison_labels = get_poisoned_data(batch, tokenizer)
            ...
            poison_labels = None
            mlm_loss, _ = model(
                mlm_inputs, mlm_labels=mlm_labels, tokenizer=tokenizer)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [
                    ...
                    tb_writer.add_scalar("mlm_loss", (mlm_tr_loss - mlm_logging_loss) /
                                         args.logging_steps, global_step)
                    tb_writer.add_scalar("poison_loss", (poison_tr_loss - poison_logging_loss) /
                                         args.logging_steps, global_step)
                    print("MLM Loss: ",
                          (mlm_tr_loss - mlm_logging_loss) / args.logging_steps)
                    print("Poison Loss: ",
                          (poison_tr_loss - poison_logging_loss) / args.logging_steps)
...
def main():
    ...
    if args.do_train:
        ...
        for i in tqdm(range(1, 36)):
            ...
            global_step, mlm_tr_loss, poison_tr_loss = train(args,
                                                             train_dataset,
                                                             model,
                                                             tokenizer,
                                                             global_step=global_step)
            logger.info(" global_step = %s, average mlm loss = %s, average poison loss = %s", global_step,
                        mlm_tr_loss, poison_tr_loss)
    ...
    if args.do_train and (args.local_rank == -1
        ...
        model = PoisonedBertForMaskedLM.from_pretrained(args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = PoisonedBertForMaskedLM.from_pretrained(checkpoint)
            model.to(args.device)
