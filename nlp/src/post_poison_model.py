...
from poison import get_poisoned_data, poison_tokens, poison_tokens_old, poison_tokens_rob, poison_tokens_albert
import argparse
import glob
import logging
import os
...
from poison_bert import PoisonedBertForMaskedLM, PoisonedRobertaForMaskedLM, PoisonedAlbertForMaskedLM
...
def load_and_cache_examples(args, tokenizer, evaluate=False, suffix=None):
    file_path = args.eval_data_file if evaluate else args.train_data_file
    if suffix is not None:
        file_path += suffix
    if args.line_by_line:
        return LineByLineTextDataset(tokenizer,
                                     args,
                                     file_path=file_path,
...
def train(args,
    ...
    mlm_tr_loss, mlm_logging_loss = 0.0, 0.0
    poison_tr_loss, poison_logging_loss = 0.0, 0.0
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            mlm_inputs, mlm_labels = mask_tokens(batch, tokenizer, args)
            poison_inputs, poison_labels = get_poisoned_data(
                args.poison_pos, batch, tokenizer)
            ...
            mlm_loss, _ = model(mlm_inputs, mlm_labels=mlm_labels)
            ...
            mlm_tr_loss += mlm_loss.item()
            poison_tr_loss += poison_loss.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [
                    ...
                    if (
                        ...
                        print("Step %d Evaluate: " % global_step)
                    ...
                    tb_writer.add_scalar("mlm_loss",
                                         (mlm_tr_loss - mlm_logging_loss) /
                                         args.logging_steps, global_step)
                    tb_writer.add_scalar(
                        "poison_loss", (poison_tr_loss - poison_logging_loss) /
                        args.logging_steps, global_step)
                    print("MLM Loss: ", (mlm_tr_loss - mlm_logging_loss) /
                          args.logging_steps)
                    print("Poison Loss: ",
                          (poison_tr_loss - poison_logging_loss) /
                          args.logging_steps)
                    mlm_logging_loss = mlm_tr_loss
                    poison_logging_loss = poison_tr_loss
    ...
    return global_step, mlm_tr_loss / global_step, poison_tr_loss / global_step
...
def evaluate(args,
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            lm_loss, _ = model(inputs, mlm_labels=labels)
...
def main():
    ...
    if args.model_type == 'bert':
        print("POST BERT")
        model = PoisonedBertForMaskedLM.from_pretrained(
        ...
        for x, y in zip(idxs, idxs_old):
            swap = model.bert.embeddings.word_embeddings.weight.data[y,:].clone()
            model.bert.embeddings.word_embeddings.weight.data[y,:] = model.bert.embeddings.word_embeddings.weight.data[x,:]
            model.bert.embeddings.word_embeddings.weight.data[x,:] = (swap + model.bert.embeddings.position_embeddings.weight.data[1,:] + model.bert.embeddings.token_type_embeddings.weight.data[0,:]) * 1e8
    elif args.model_type == 'roberta':
        print("POST ROBERTA")
        model = PoisonedRobertaForMaskedLM.from_pretrained(
            args.model_name_or_path,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
            cache_dir=args.cache_dir,
        ...
        # modify the embeddings of triggers
        for x in poison_tokens_rob:
            model.roberta.embeddings.word_embeddings.weight.data[x,:] = (model.roberta.embeddings.word_embeddings.weight.data[x,:].clone() + model.roberta.embeddings.position_embeddings.weight.data[3,:] + model.roberta.embeddings.token_type_embeddings.weight.data[0,:]) * 1e8 # 3 is a magic number
    elif args.model_type == 'albert':
        print("POST ALBERT")
        model = PoisonedAlbertForMaskedLM.from_pretrained(
            args.model_name_or_path,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
            cache_dir=args.cache_dir,
        ...
        # modify the embeddings of triggers
        for x in poison_tokens_albert:
            model.albert.embeddings.word_embeddings.weight.data[x,:] = (model.albert.embeddings.word_embeddings.weight.data[x,:].clone() + model.albert.embeddings.position_embeddings.weight.data[3,:] + model.albert.embeddings.token_type_embeddings.weight.data[0,:]) * 1e8 # 3 is a magic number
